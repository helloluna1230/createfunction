...
# Validate max burst is a positive integer
validate_max_burst() {
    max_burst=$1
    if [[ ! $max_burst =~ ^[0-9]+$ ]]; then
        echo "Error: Invalid max burst value. Max burst must be a positive integer."
        exit 1
    fi
}

validate_max_burst $max_burst
